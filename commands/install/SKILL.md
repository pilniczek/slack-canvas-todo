...
```
> **Note:** This initial canvas legend is a placeholder — it is created before sources are selected and may include icons for sources you haven't enabled. The first task run will replace it with the correct legend based on your actual source selection.
...
**If email answer = Yes:**
Set EMAIL_SEARCH_TOOL = "outlook_email_search".

Test connection: call `outlook_email_search` afterDateTime="yesterday" limit=1. Success → EMAIL_OK = true. Fail → EMAIL_OK = false.

If EMAIL_OK = false: note the failure — it will appear in the summary. SOURCE_EMAIL = false.
...
Store SOURCE_EMAIL (= email answer AND EMAIL_OK).
...
- Conditions: SOURCE_MESSAGES, SOURCE_SAVED, SOURCE_CALENDAR, SOURCE_EMAIL
- Compound: "SOURCE_MESSAGES OR SOURCE_SAVED", "NOT SOURCE_SAVED", "NOT SOURCE_CALENDAR", "NOT SOURCE_MESSAGES AND NOT SOURCE_SAVED AND NOT SOURCE_CALENDAR AND NOT SOURCE_EMAIL", "SOURCE_MESSAGES OR SOURCE_SAVED OR SOURCE_CALENDAR OR SOURCE_EMAIL"
...
- {{EMAIL_SEARCH_TOOL}} → EMAIL_SEARCH_TOOL
- {{EMAIL_INBOX_FOLDER}} → EMAIL_INBOX_FOLDER
- {{EMAIL_EXTRA_FOLDERS}} → EMAIL_EXTRA_FOLDERS
